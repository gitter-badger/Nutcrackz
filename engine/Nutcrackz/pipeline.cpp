...
const Matrix4f& Pipeline::GetViewTrans()
{
	...
	Vector3f camTarget = Vector3f(m_camera.Target.x, m_camera.Target.y, m_camera.Target.z);
	Vector3f camUp = Vector3f(m_camera.Up.x, m_camera.Up.y, m_camera.Up.z);

	CameraRotateTrans.InitCameraTransform(camTarget, camUp);
