...
void Capsule::init(b2World *world, glm::vec2 &position, glm::vec2 &dimensions, ColorRGBA8 &color, float density, float friction, bool fixedRotation)
{
...
void Capsule::drawDebug(DebugRenderer &debugRenderer)
{
	//Draw box
	glm::vec4 destRect;
	...
	debugRenderer.drawBox(destRect, m_color, m_body->GetAngle());

	//Draw circles
	debugRenderer.drawCircle(glm::vec2(destRect.x + m_dimensions.x / 2.0f, destRect.y), m_color, m_dimensions.x / 2.0f);
	debugRenderer.drawCircle(glm::vec2(destRect.x + m_dimensions.x / 2.0f, destRect.y + destRect.w), m_color, m_dimensions.x / 2.0f);
}