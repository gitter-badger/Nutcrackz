...
void DebugRenderer::drawTriangle(const glm::vec2 &a, const glm::vec2 &b, const glm::vec2 &c, const ColorRGBA8 color)
{
	int i = m_verts.size();
	...
	//m_verts[i + 2].position = glm::vec2(b.x, c.y);
	//m_verts[i + 2].color = color;

	m_verts[i + 2].position = b;
	m_verts[i + 2].color = color;

	m_verts[i + 3].position = c;
	m_verts[i + 3].color = color;
	
	m_verts[i + 4].position = c;
	m_verts[i + 4].color = color;

	//m_verts[i + 5].position = glm::vec2(a.x, c.y);
	//m_verts[i + 5].color = color;
	
	m_verts[i + 5].position = a;
	m_verts[i + 5].color = color;
...
void DebugRenderer::drawPrimitiveBox(const glm::vec4 &destRect, const ColorRGBA8 &color, float angle)
{
	int i = m_verts.size();
...
void DebugRenderer::drawBox(const glm::vec4 &destRect, const ColorRGBA8 &color, float angle)
{
	...
	m_indices.reserve(m_indices.size() + 10);
	m_indices.push_back(i);
	m_indices.push_back(i + 1);
...
void DebugRenderer::drawOctagon(const glm::vec2 &center, const ColorRGBA8 &color, float radius)
{
...
void DebugRenderer::render(const glm::mat4 &projectionMatrix, float lineThickness)
{
	m_program.use();
